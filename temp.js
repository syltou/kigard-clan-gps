...
function updateComponentsFilter() {
    ...
    let bloc = document.getElementById("bloc");
    let table = bloc.getElementsByTagName("table")[0];
    let list_icons = '<blockquote id="ingredients" class="bloc"><strong>Ingrédients</strong><br>';
    for(var i=0;i<ingredients.length;i++) {
        list_icons += '<span><a href="#" data-comp="' + ingredients[i].split('.')[0] + '"><img src="images/items/' + ingredients[i] + '" class="item"> </a>';
        // if(i<ingredients.length-1) list_icons += '<img src="images/interface/puce_small.gif" alt="">';
        list_icons += '</span>';
    }
    list_icons += '</blockquote>';

    let blockquote = document.getElementById("ingredients");
...
function updateTableTitle() {
    ...
    $("table:first").children("tbody:first").children("tr:first").children("td:first")[0].innerText = displayed_lines.length + " formules après filtrage";
}
